...
namespace cg::source {
    ...
    // @brief:
    // Алиас может превращаться неявно в тип
    // Сам тип это просто имя алиаса без ничего, ни шаблонов, ни квалификаторов
    // чистый тип
    // Кстати сам тип хоть и является шаблонной сущностью, в валидации не нуждается
    // Так как тип всегда использует шаблоны в качестве аргументов
    // А сам аргумент может быть и не референсом типа.
    // Можно было бы конечно сделать свои методы и поля, но зачем? если
    // уже есть удобный миксин, так что - имейте ввиду!
